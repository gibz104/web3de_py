...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def contract_get_source_code(addr):
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def account_get_ether_balance(addr):
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def account_get_normal_transactions(addr):
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def account_get_internal_transactions(addr):
    # Returns MAX 10k transactions
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def account_get_ERC20_transfers(addr):
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def account_get_ERC721_transfers(addr):
...
@sleep_and_retry
@limits(calls=RATE_LIMIT, period=RATE_LIMIT_PERIOD_SECONDS)
def transactions_get_internal_transactions(tx):
